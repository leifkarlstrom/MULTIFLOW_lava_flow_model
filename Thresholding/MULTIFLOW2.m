function [Influence, FlowMap, INFLUENCE_THRESHOLD] = MULTIFLOW2(DEMfiltered, p, threshold_type)
workspace;
...
V = 917000000; % volume of 1984 lava flow
%INFLUENCE_THRESHOLD = V./(DISTANCE.^3)+15;
INFLUENCE_THRESHOLD = p.a*(DISTANCE.^p.b) - p.c;

% a1 and b1 from Least-squares plane regression
a1= -0.6939; 
b1= 0.426 ;
slopefactor = 1-((a1/b1)*(Y_dist./X_dist)) ;

if threshold_type=="length"
    INFLUENCE_THRESHOLD = p.a*(DISTANCE.^p.b) - p.c ; %p.c;
elseif threshold_type=="scaleheight"
    INFLUENCE_THRESHOLD = p.a*(p.H.^p.b) - p.c ;
elseif threshold_type=="slope"
    INFLUENCE_THRESHOLD = p.a*(slopefactor.^p.b) - p.c ;
end
